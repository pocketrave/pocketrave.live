...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-neutral-950/5 backdrop-blur-xs border-b border-white/5 flex items-center h-14 md:h-20 font-orbitron">
      <div className="max-w-7xl mx-auto px-4 md:px-16 w-full flex items-center justify-between">
        <Link 
          href="/" 
          className={`text-base sm:text-xl md:text-4xl lg:text-6xl font-bold tracking-tight ${pathname === '/' ? 'text-red-600' : 'text-red-600 hover:text-blue-600 transition-colors'}`}
          style={{ fontVariationSettings: '"wght" 700' }}
        >
          <span>Pocket Rave</span>
          <BusyIndicator className="ml-1 sm:ml-2 opacity-80" />
        </Link>
